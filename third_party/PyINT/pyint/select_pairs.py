...
#    date12_list_out = date12_list0[(ifgram_tbase_list_abs < float(templateDict['max_tb'])) & (ifgram_pbase_list_abs < float(templateDict['max_sb']))]
    date12_list_out = date12_list0[(ifgram_tbase_list_abs < float(templateDict['max_tb'])) & (ifgram_tbase_list_abs > float(templateDict['min_tb'])) & (ifgram_pbase_list_abs < float(templateDict['max_sb']))]
...
#    ifgram_pbase_list_out = ifgram_pbase_list[(ifgram_tbase_list_abs < float(templateDict['max_tb'])) & (ifgram_pbase_list_abs < float(templateDict['max_sb']))]
    ifgram_tbase_list_out = ifgram_tbase_list[(ifgram_tbase_list_abs < float(templateDict['max_tb'])) & (ifgram_tbase_list_abs > float(templateDict['min_tb'])) & (ifgram_pbase_list_abs < float(templateDict['max_sb']))]
    ifgram_pbase_list_out = ifgram_pbase_list[(ifgram_tbase_list_abs < float(templateDict['max_tb'])) & (ifgram_tbase_list_abs > float(templateDict['min_tb'])) & (ifgram_pbase_list_abs < float(templateDict['max_sb']))]
...
def cmdLineParse():
    ...
    parser.add_argument('--min-tb', dest='minTB', type=str, help='minmum temporal baseline')
...
EXAMPLE = '''
    Examples: 
            ...
            select_pairs.py PacayaT163TsxHhA --max-tb 300 --min-tb 200 --max-sb 100
...
def main(argv):
    ...
    slc_list0 = [os.path.basename(fname) for fname in sorted(glob.glob(rslcDir + '/*'))]
    slc_list = []
    for k0 in slc_list0:
        if ut.is_number(k0):
            slc_list.append(k0)
    ...
    for kk in range(len(slc_list)):
        ...
        str_slc = rslcDir + "/" + slc_list[kk] +"/" + slc_list[kk] + slc0
        str_slc_par = rslcDir + "/" + slc_list[kk] +"/" + slc_list[kk] + slcpar0
        SLCfile.append(str_slc)
        SLCParfile.append(str_slc_par)       

    RefPar = rslcDir + "/" + masterDate +"/" + masterDate + slcpar0
