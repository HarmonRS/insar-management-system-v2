...
INTRODUCTION = '''
-------------------------------------------------------------------  
       Correct atmospheric phase for one project using GAMMA.
   
'''

EXAMPLE = '''
    Usage: 
            atm_correction_gamma_all.py projectName
            atm_correction_gamma_all.py projectName --parallel 4
            atm_correction_gamma_all.py projectName --parallel 4 --ifgramList-txt /test/ifgram_list.txt
-------------------------------------------------------------------  
'''


def cmdLineParse():
    parser = argparse.ArgumentParser(description='Unwrap differential interferograms for one project using GAMMA.',\
...
#    ifgList = ifgList0[:,0]
    ...
    for i in range(len(ifgList)):
        ...
        cmd0 = ['atm_correction_gamma.py',projectName, m0, s0]
        unw_file0 = ifgDir + '/' + ifgList[i] + '/' + ifgList[i] + '_' + rlks + 'rlks.diff_filt.atmcor.unw.bmp'
        data0 = [cmd0,err_txt]
        
        k00 = 0
        if os.path.isfile(unw_file0):
            if os.path.getsize(unw_file0) > 0:
                k00 = 1
        if k00==0:
