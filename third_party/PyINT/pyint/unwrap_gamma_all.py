...
INTRODUCTION = '''
-------------------------------------------------------------------  
       Unwrap differential interferograms for one project using GAMMA.
   
'''

EXAMPLE = '''
    Usage: 
            unwrap_gamma_all.py projectName
            unwrap_gamma_all.py projectName --parallel 4
            unwrap_gamma_all.py projectName --parallel 4 --ifgramList-txt /test/ifgram_list.txt
-------------------------------------------------------------------  
'''
...
#    ifgList = ifgList0[:,0]
    ...
    if failures:
        raise RuntimeError('\n\n'.join(failures))
    print("Unwrap differential interferograms for project %s is done! " % projectName)
