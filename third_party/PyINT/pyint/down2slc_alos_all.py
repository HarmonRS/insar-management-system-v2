...
def main(argv):
    ...
    raw_file_list = glob.glob(downDir + '/ALPSRP*.zip')
    slc_dir = scratchDir + '/' + projectName + '/SLC'
    if not os.path.isdir(slc_dir):
        os.mkdir(slc_dir)

    err_txt = scratchDir + '/' + projectName + '/unzip_alos_all.err'
    if os.path.isfile(err_txt): os.remove(err_txt)
########get the ALOS date name ###############
    data_para = []
    date_list = []
    for i in range(len(raw_file_list)):
        cmd0 = ['unzip',raw_file_list[i],'-d',downDir]
        call_str='unzip ' + raw_file_list[i] + ' -d ' + downDir
        os.system(call_str)
        workflow_file=raw_file_list[i].split(".")[0] + '.' +raw_file_list[i].split(".")[1]
        os.chdir(workflow_file)
        tempfile=workflow_file + '/' + 'workreport'
        tempDict=ut.update_template(tempfile)
        SeneceData = tempDict['Img_SceneCenterDateTime'].split('"')[1].split(' ')[0]
        SceneID    = tempDict['Scs_SceneID']
        os.chdir(downDir)
        call_str= 'mv ' + workflow_file + ' ' + SeneceData 
        os.system(call_str)
        data0=[SeneceData, err_txt]       
    ...
    err_txt = scratchDir + '/' + projectName + '/down2slc_alos_all.err'
    if os.path.isfile(err_txt): os.remove(err_txt)
    
    data_para = []
    for i in range(len(date_list)):
        cmd0 = ['Down2SLC_ALOS.py',projectName,date_list[i]]
        data0 = [cmd0,err_txt]
        data_para.append(data0)
