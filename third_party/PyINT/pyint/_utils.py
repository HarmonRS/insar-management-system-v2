...
# 无条件清理 OMP_PLACES / OMP_PROC_BIND，避免 libgomp 报错
for _k in ('OMP_PLACES', 'OMP_PROC_BIND'):
...
def update_template(template_file):
    ...
    ######### download using ASF API ######
    templateDict['asf_username'] = ''           # ASF username (can be read from ~/.netrc)
    templateDict['asf_password'] = ''           # ASF password (can be read from ~/.netrc)
    templateDict['bbox'] = ''                   # bounding box: min_lon,min_lat,max_lon,max_lat
    templateDict['shapefile'] = ''              # shapefile path for spatial filtering
    templateDict['relative_orbit'] = ''         # relative orbit number (optional)
    templateDict['frame'] = ''                  # frame number (optional)
    templateDict['flight_direction'] = 'a'      # flight direction: a=ascending, d=descending, all
    templateDict['acquisition_mode'] = 'IW'     # acquisition mode: IW, EW, SM, all
    ...
    templateDict['dem_num_workers'] = '4'  # number of parallel workers for DEM download
    templateDict['dem_source'] = 'copernicus'  # DEM data source: copernicus or nasadem
    ...
    #### unwrap phase ###########
    templateDict['auto_unw'] = '1'          # auto reference-point selection
    templateDict['make_mask'] = '1'         # coherence-mask switch for unwrap
    templateDict['init_flag'] = '1'         # set reference-point phase to 0.0
    templateDict['r_refer'] = '-'           # range reference pixel for manual unwrap
    templateDict['a_refer'] = '-'           # azimuth reference pixel for manual unwrap
    templateDict['mcf_triangular'] = '0'    # triangular type of mcf [0: regular; 1: delaunay;] 
    templateDict['unwrap_patr'] = '4'
    templateDict['unwrap_pataz'] = '4'
    templateDict['unwrapThreshold'] = '0.1'  # minimum coherence used for unwrap
    
    #### geocode #########
    templateDict['geo_interp'] = '0' #  [0: nearest; 1: bicubic spline]
    templateDict['geocode_products'] = 'hyp3,licsbas'  # 产品类型: hyp3,licsbas,pot (逗号分隔多选)
    ...
    templateDict['gamma2licsbas_all'] = '0'         # legacy conversion stage switch
    templateDict['gamma2licsbas_all_parallel'] = '1'
    templateDict['hyp3format_all'] = '0'            # legacy conversion stage switch
    templateDict['hyp3format_all_parallel'] = '1'
    
    ############## GACOS atmospheric correction #############
    templateDict['gacos_correction'] = '0'            # if 1, apply GACOS atmospheric correction
    templateDict['gacos_all_parallel'] = '1'          # multi-processor number used for GACOS correction
    templateDict['gacos_dir'] = ''                    # directory for GACOS ZTD files (default: projectName/GACOS)
    templateDict['gacos_email'] = ''                  # email for GACOS auto-download
    templateDict['gacos_email_user'] = ''             # IMAP username
    templateDict['gacos_email_pass'] = ''             # IMAP authorization code
    templateDict['gacos_email_host'] = 'imap.163.com' # IMAP server host
    templateDict['gacos_email_port'] = '993'          # IMAP server port
    templateDict['gacos_email_ssl'] = '1'             # Use SSL
    templateDict['gacos_check_interval'] = '60'       # Email check interval (seconds)
    
    ############## Pixel Offset Tracking (POT) #############
    templateDict['pot_all'] = '0'                    # if 1, run offset tracking
    templateDict['pot_all_parallel'] = '1'           # multi-processor number used
    templateDict['pot_rstep'] = '50'                 # range step in SLC pixels
    templateDict['pot_azstep'] = '10'                # azimuth step in SLC pixels
    templateDict['pot_rwin'] = '256'                 # range window size (Round 1)
    templateDict['pot_azwin'] = '128'                # azimuth window size (Round 1)
    templateDict['pot_rwin2'] = '128'                # range window size (Round 2)
    templateDict['pot_azwin2'] = '64'                # azimuth window size (Round 2)
    templateDict['pot_ovr'] = '1'                    # SLC oversampling factor (1=no deramp needed)
    templateDict['pot_snr_thresh'] = '0.01'          # SNR threshold for offset_pwr_tracking
    templateDict['pot_ccp_thresh'] = '0.05'          # cross-correlation threshold for masking
    templateDict['pot_roff_min'] = '-50'             # min range offset (pixels) for masking
    templateDict['pot_roff_max'] = '50'              # max range offset (pixels) for masking
    templateDict['pot_azoff_min'] = '-20'            # min azimuth offset (pixels) for masking
    templateDict['pot_azoff_max'] = '20'             # max azimuth offset (pixels) for masking
    templateDict['pot_drange_thresh'] = '2.0'        # deviation threshold range (pixels)
    templateDict['pot_dazimuth_thresh'] = '1.0'      # deviation threshold azimuth (pixels)
    templateDict['pot_median_win'] = '7'             # median filter window size
    templateDict['pot_median_nmin'] = '15'           # min valid samples for median filter
    templateDict['pot_two_rounds'] = '1'             # 1=two-round estimation, 0=single round
    templateDict['pot_geocode'] = '1'                # 1=geocode displacement results
    templateDict['pot_disp_max'] = '100'             # max displacement for BMP display (meters)
