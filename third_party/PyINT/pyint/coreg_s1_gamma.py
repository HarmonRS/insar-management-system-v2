...
#  Definition of file
    ...
    TEST = workDir + '/' + Mdate +  '_' + Sdate +'.diff'
    ...
    if k0 == 0:
        if not Mdate==Sdate:
#            call_str = 'S1_coreg_TOPS ' + SLC1_INF_tab1 + ' ' + Mdate + ' ' + SLC2_INF_tab + ' ' + Sdate + ' ' + RSLC_tab + ' ' + HGTSIM + ' ' + rlks + ' ' + azlks + ' - - 0.8 0.01 1.2 1'
            call_str = 'ScanSAR_coreg.py ' + SLC1_INF_tab1 + ' ' + Mdate + ' ' + SLC2_INF_tab + ' ' + Sdate + ' ' + RSLC_tab + ' ' + HGTSIM + ' ' + rlks + ' ' + azlks + ' --cc 0.8 --fraction 0.01 --ph_stdev 0.8 --num_ovr 0 --no_check '
            print(call_str)
            ...
            # 检查 ScanSAR_coreg.py 是否成功生成 .rslc（不检查返回码，spectral diversity 步骤可能报错但 RSLC 已生成）
            if not os.path.isfile(srslc) or os.path.getsize(srslc) == 0:
                print('ERROR: ScanSAR_coreg.py did not produce a valid ' + srslc)
                sys.exit(1)
