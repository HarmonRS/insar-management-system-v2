...
def main(argv):
    ...
    os.system(call_str)
    call_str = "xyz2grd lv_theta.phase_swap -Glv_theta.grd -Ddegree/degree/cm/1/0/=/= -R" + West + "/" + East + "/" +South + "/" +North + " -I" + postx + " -ZTLf -N0"
    os.system(call_str)
    call_str = "grdmath 90 lv_theta.grd 3.1415926 DIV 180 MUL SUB = lv_theta_final.grd"
    os.system(call_str)
    call_str = "grdmath 90 lv_theta_final.grd SUB = lv_elev.grd"
    os.system(call_str)
    call_str = "grdsample lv_elev.grd -Glv_elev_3c.grd   -I3c"
   # call_str = "grdsample lv_theta_final.grd -Glv_elev_3c.grd   -I3c"
    os.system(call_str)
    call_str = "grd2xyz lv_elev_3c.grd -ZTLf -N0 > " + Mdate + '-' + Sdate + '.gacos.elev'
    os.system(call_str)


    call_str = "swap_bytes " + unw+ " unw.phase_swap 4 >dinsar.log "
    os.system(call_str)
    call_str = "xyz2grd unw.phase_swap -Gunw_f.grd -Ddegree/degree/cm/1/0/=/= -R" + West + "/" + East + "/" +South + "/" +North + " -I" + postx +" -ZTLf -N0"
    os.system(call_str)
    call_str = "grdsample unw_f.grd -Gunw.grd   -I3c"
    os.system(call_str)
    call_str = "grd2xyz unw.grd -ZTLf -N0 > " + Mdate + '-' + Sdate + '.gacos.unw'
    os.system(call_str)
    call_str = "grdinfo unw.grd -C | awk '{print $10}' "
    Width=subprocess.getstatusoutput(call_str)[1]
    call_str = "grdinfo unw.grd -C | awk '{print $11}' "
    line=subprocess.getstatusoutput(call_str)[1]
    call_str = "grdinfo unw.grd -C | awk '{print $2}' "
    West=subprocess.getstatusoutput(call_str)[1]
    call_str = "grdinfo unw.grd -C | awk '{print $5}' "
    North=subprocess.getstatusoutput(call_str)[1]
